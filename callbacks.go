...
func (exec *sidecarExecutor) KillTask(driver executor.ExecutorDriver, taskID *mesos.TaskID) {
	...
	drainingDuration := exec.config.SidecarDrainingDuration
	if exec.containerConfig != nil && exec.containerConfig.Config != nil {
		if durationLabel, ok := exec.containerConfig.Config.Labels["SidecarDrainingDuration"]; ok {
			d, err := time.ParseDuration(durationLabel)
			if err != nil {
				log.Errorf("Failed to parse SidecarDrainingDuration %q: %s. Using default: %s", durationLabel, err, drainingDuration)
			} else {
				drainingDuration = d
			}
		}
	...
	if shouldCheckSidecar(exec.containerConfig) {
		exec.drainService(drainingDuration)
	}
